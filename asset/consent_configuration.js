...
function saveConsentChoices(choices) {
  var bannerSuffix = window.aCBm ? window.aCBm.cookieBanner ? window.aCBm.cookieBanner.getBannerSuffix() || '' : '' : '';
  ...
  fetch('https://www.allbestview.com/set-consent', {
    ...
    .then(function(data) {
      localStorage.setItem('cookie_consent' + bannerSuffix, JSON.stringify(data.consents));
...
function syncConsentFromCookie() {
  ...
  if (typeof cookieConsent === 'string') {
    try {
      ...
      // Validasi properti yang diharapkan ada
      if (typeof consent === 'object' && consent !== null) {
        ...
        console.warn(`Isi ${cookieKey} bukan objek consent yang valid:`, consent);
    ...
    console.warn(`${cookieKey} tidak ditemukan atau bukan string valid`);
    initializeDefaultConsent(bannerSuffix);
  }
}

// Fungsi fallback untuk inisialisasi nilai default consent
